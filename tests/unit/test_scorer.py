...
class TestMatchScorer:
    ...
    def test_score_bounds(self, scorer):
        ...
        alibaba = AlibabaProduct(
            item_id="500", title="T", price=1.0, min_order_qty=1, matched_score=100.0
        )
        result = scorer.score_match(amazon, alibaba)
...
async def test_parallel_matching_returns_match_results(config, mock_storage):
    ...
    mock_scorer = MatchScorer(config)
    ...
    mock_matcher.search_and_match = AsyncMock(return_value=supplier)

    # 使用 orchestrator 的 matching service
    engine._ensure_orchestrator()
    engine._orchestrator.matching.matcher = mock_matcher
    engine._orchestrator.matching.scorer = mock_scorer
