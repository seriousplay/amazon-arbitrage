...
_custom_terms = [
    '耐咬', '强力咀嚼', '发声器', '自动饮水', '不锈钢', '活水器',
    '猫砂盆', '带盖', '封闭式', '大型犬', '小型犬', '中型犬',
    '宠物床', '猫爬架', '猫抓板', '狗咬胶', '磨牙棒', '宠物食碗',
    '航空箱', '胸背带', '牵引绳', '宠物包', '尿垫', '除臭',
    '饮水机', '食碗', '砂盆', '宠物窝',
]
...
class FuzzyMatcher:
    ...
    def __init__(self, language: str = "zh"):
        ...
        self.pet_synonyms = {
            ...
            'ball': '球',
            'treat': '零食',
            'treat dispenser': '零食分配器 漏食器',
            'feeder': '喂食器',
            'water fountain': '饮水机 活水器',
            'bowl': '碗 食盆',
            'food bowl': '食碗',
            'litter box': '砂盆 猫砂盆',
            'litter': '猫砂',
            'scratching': '抓挠 磨爪 猫抓板',
            'cat tree': '猫爬架',
            'cat condo': '猫窝 猫屋',
            'crate': '航空箱 托运箱 笼子',
            'carrier': '携带包 宠物包',
        ...
        custom_terms = [
            # 核心复合词（防止 jieba 错误切分）
            '耐咬', '强力咀嚼', '发声器', '自动饮水', '自动饮水器', '不锈钢', '活水器',
            '猫砂盆', '带盖', '封闭式', '大型犬', '小型犬', '中型犬',
            '宠物床', '猫爬架', '猫抓板', '狗咬胶', '磨牙棒', '宠物食碗',
            '航空箱', '胸背带', '牵引绳', '宠物包', '尿垫', '除臭',
            # 补充
            '宠物', '食碗', '碗', '饮水', '发声', '球', '猫', '狗',  # 常见单字词强制保留
            '互动', '娱乐', '自动', '智能', '感应',
        ]
        ...
        self.en_variations = {
            # 复数形式
            'chewers': 'chewer', 'toys': 'toy', 'bowls': 'bowl',
            'fountains': 'fountain', 'boxes': 'box', 'litters': 'litter',
            'brushes': 'brush', 'collars': 'collar', 'harnesses': 'harness',
            'leashes': 'leash', 'beds': 'bed', 'mats': 'mat', 'pads': 'pad',
            'carriers': 'carrier', 'crates': 'crate', 'balls': 'ball',
            'bones': 'bone', 'sticks': 'stick', 'chews': 'chew',
            'squeakers': 'squeaker', 'bones': 'bone',
            # 现在分词/动词形式
            'chewing': 'chew', 'squeaking': 'squeak',
            # 所有格/形容词
            'pet': 'pet',  # 本身不变
        }
        self.stop_words_en = {
            'the', 'a', 'an', 'for', 'with', 'and', 'or', 'of', 'in', 'on',
            'new', 'best', 'official', 'original', 'genuine', 'pack', 'set',
            'count', 'pc', 'pcs', 'x', 'premium', 'quality', 'heavy', 'duty',
            'pro', 'professional', 'advanced', 'ultimate', 'plus', 'ultra',
            'deluxe', 'upgraded', 'version', 'edition', 'collection',
            '2024', '2025',
        }
        self.stop_words_zh = {
            '新款', '正品', '官方', '原装', '包邮', '特价', '促销', '套装',
            '清仓', '限时', '折扣', '特惠', '爆款', '热销', '推荐', '畅销',
            '个', '只', '条', '袋', '盒', '箱', '件', '套', '支',
            '毫升', '克', '公斤', '斤', '升', '米', '厘米', '毫米',
        }
        ...
        self.semantic_norm = {
            # === 宠物类型归一化 ===
            '狗': '狗', '狗狗': '狗', '犬类': '狗', '宠物狗': '狗',
            '犬': '狗', '大型犬': '狗', '小型犬': '狗', '中型犬': '狗',
            '猫': '猫', '猫咪': '猫', '宠物猫': '猫', 'feline': '猫',
            # '宠物': '宠物',  # 保留原词（避免过度归一化）
            
            # === 功能特性归一化 ===
            '耐咬': '耐咬', '强力咀嚼': '耐咬', '咀嚼': '耐咬', '咬嚼': '耐咬', '啃咬': '耐咬',
            '发声': '发声', '发声器': '发声', '吱吱响': '发声', 'squeaky': '发声',
            '互动': '互动', '交互': '互动',
            '自动': '自动', '智能': '自动', '感应': '自动',
            '饮水': '饮水', '活水': '饮水', '循环': '饮水',
            '除臭': '除臭', '防臭': '除臭', '祛味': '除臭',
            '防水': '防水', '防漏': '防水', '防湿': '防水',
            '防滑': '防滑',
            '可调节': '可调节', '调节': '可调节', '伸缩': '可调节',
            '可折叠': '可折叠', '折叠': '可折叠', '便携': '便携',
            '耐用': '耐用', '耐咬': '耐用', '加固': '耐用', '结实': '耐用',
            '柔软': '柔软', '舒适': '柔软',
            '静音': '静音', '无声': '静音', '低噪音': '静音',
            
            # === 材质归一化 ===
            '不锈钢': '不锈钢', '不锈': '不锈钢', '钢': '不锈钢', '钢铁': '不锈钢', '金属': '不锈钢', '合金': '不锈钢',
            '塑料': '塑料', '塑胶': '塑料', '树脂': '塑料',
            '棉': '棉', '纯棉': '棉', '棉质': '棉',
            '抓绒': '绒', '绒': '绒', '毛绒': '绒',
            '尼龙': '尼龙',
            ...
            # === 复合词归一化 ===
            '饮水机': '饮水', '自动饮水': '饮水', '自动饮水器': '饮水', '活水器': '饮水',
            '食碗': '碗', '饭盆': '碗', '宠物食碗': '碗', '碗': '碗',
            '猫砂盆': '砂盆', '猫厕所': '砂盆',
            '发声球': '发声 球',
            '宠物玩具': '玩具',
            '耐咬玩具': '耐咬 玩具',
    ...
    def _extract_specs(self, text: str) -> Dict[str, str]:
        ...
        # 颜色
        color_match = re.search(r'\b(Red|Blue|Green|Yellow|Black|White|Gray|Brown|Pink|Purple)\b', text, re.IGNORECASE)
        if color_match:
            specs['color'] = color_match.group(0).lower()
        
        # 材质
        material_patterns = [
            (r'\b(stainless steel|steel|plastic|cotton|fleece|nylon|rubber|silicone|leather|fabric|mesh)\b', 'material'),
        ]
    ...
    def title_similarity(self, title_a: str, title_b: str) -> Tuple[float, Dict]:
        ...
        conflicts = sum(1 for k in critical_keys if k in specs_a and k in specs_b and specs_a[k] != specs_b[k])
        if conflicts > 0:
