...
class DelayManager:
    ...
    def wait(self, jitter: float = 0.5) -> None:
        ...
        if elapsed < target_delay:
            ...
            logger.debug(f"Delay: 等待 {sleep_time:.2f}s (最小 {self.min_delay}s, 最大 {self.max_delay}s)")
...
def retry_request(max_retries: int = 3, 
                ...
                try:
                    ...
                    if hasattr(result, 'status_code'):
                        ...
                        if status in retry_codes and attempt < max_retries:
                            ...
                            logger.warning(f"HTTP {status}，{wait_time:.1f}s后重试（第{attempt+1}次）")
