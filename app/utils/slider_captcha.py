...
class SliderCaptchaSolver:
                ...
                if bg_elem:
                    ...
                    if slider_elem:
                        ...
                        if bg_img is not None and slider_img is not None:
                            logger.info(f"成功截取背景图 {bg_img.shape} 和滑块图 {slider_img.shape}")
    ...
    @staticmethod
    def generate_bezier_trajectory(distance: int, 
                                   start_x: int = 0, 
                                   start_y: int = 0,
        ...
        logger.debug(f"生成轨迹: 起点({start_x},{start_y}) → 终点({end_x},{end_y}), {len(points)} 个点")
        return points
    
    @staticmethod
    def generate_human_like_trajectory(distance: int, 
                                       start_x: int = 0, 
                                       start_y: int = 0) -> List[Tuple[int, int]]:
        """
        更复杂的人类轨迹模拟（带速度曲线）
    ...
    async def perform_slide(self, 
                           slider_selector: str,
                           distance: int,
                           trajectory: Optional[List[Tuple[int, int]]] = None) -> bool:
        """
        执行滑块拖动操作
    ...
    async def solve(self, 
                    ...
                    bg_selector: Optional[str] = None) -> SliderResult:
        """
        主流程：自动检测并破解滑块验证码
